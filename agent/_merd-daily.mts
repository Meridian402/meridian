...
// The FLOW PACK: the desk's own per-pool sensors (sell-share, swap
// acceleration, velocity, drawdown from the recent peak). This is the analyst
// edge: nobody else publishes flow reads for this chain's pools, and every
// number here is already public on the site's own API, so citing it leaks
// nothing the site does not.
let flowLines: string[] = [];
try {
  const dw: any = await j("/api/dump-watch");
  const bleedBy = new Map<string, any>((dw?.bleed ?? []).map((b: any) => [b.symbol, b]));
  flowLines = (dw?.readings ?? []).map((r: any) => {
    const b = bleedBy.get(r.symbol);
    const drop = b?.drawdownPct != null ? `, ${b.drawdownPct}% off its ${b.hours}h peak` : "";
    return `${r.symbol}: ${r.swaps} swaps in the window, sell-share ${r.recentSellSharePct}%, sell acceleration ${r.accel}x, velocity ${r.velPct}%/hr${drop} (${r.reason})`;
  });
} catch { /* the flow read degrades, never blocks */ }
...
// an empty memory never blocks a post.
let memoryLines: string[] = [];
try {
  const rawJ: any = await j("/api/desk-journal");
  const entries: any[] = rawJ?.entries ?? [];
  const recent24 = entries.filter((e) => (e?.ts ?? 0) > Date.now() - 24 * 3600e3);
  const pick = recent24.filter((e) => e.kind === "stop-loss").pop()
    ?? recent24.filter((e) => e.kind === "collect").pop()
    ?? recent24.pop();
  if (pick) {
    const pool = pick.pool ?? pick.venue ?? pick.from;
    const r: any = await j(`/api/learn/recall?pool=${encodeURIComponent(pool ?? "")}&kind=${encodeURIComponent(pick.kind ?? "rotate")}`);
    memoryLines = (r?.memories ?? []).slice(0, 4);
  }
} catch { /* memory degrades, never blocks */ }
...
const next = roadmap();
// Ecosystem NEWS the operator verified and wants a practitioner's take on:
// a curated file, one line per item, each starting YYYY-MM-DD. Only fresh
// lines (3 days) reach the prompt, so a stale file quietly stops mattering
// instead of aging into false "news". Added 2026-08-31 for the Robinhood
// agentic-trading launches; see X-OPS.md.
function freshNews(): string[] {
  try {
    const raw = readFileSync(new URL("./merd-news.md", import.meta.url), "utf8");
    const cutoff = Date.now() - 3 * 24 * 3600e3;
    return raw.split("\n").map((l) => l.replace(/^\s*-\s*/, "").trim()).filter((l) => {
      const m = l.match(/^(\d{4}-\d{2}-\d{2})\s/);
      return m ? new Date(m[1] + "T12:00:00").getTime() >= cutoff : false;
    });
  } catch { return []; }
}
...
- MARKET: an on-chain flow read from your SENSORS above, the analyst beat only this account can write: which pool the sellers are leaning on, how fast it is accelerating, what your desk did about it. First person, through the desk. Observation and reaction ONLY: never a prediction, never advice, never "about to", and never where a resting order, floor, or next move of yours sits. Only when the sensors show a tape that actually moved (a bleed off a peak, a one-sided window, an acceleration spike); a calm tape is not a MARKET post
- NEWS: a practitioner's read on ONE item from the ecosystem news above, from the seat of an agent already doing the thing the news is about. What it actually means and what it changes, told plainly. Never imply affiliation, partnership, or endorsement, never tag the company, never advice. Only exists while the news list above is non-empty
...
Ground every brief in something real above.`;
  ...
  const types = new Set(["PRODUCT", "MARKET", "NEWS", "SHIPPED", "NEXT", "PROGRESS", "CULTURE", "TOKEN", "ASIDE", "DESK", "EARNINGS"]);
