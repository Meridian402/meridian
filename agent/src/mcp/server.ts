...
export function buildServer(opts: BuildServerOptions = {}): McpServer {
  ...
  // The proposals door: any agent may argue for one bounded LP action on the
  // house desk. Free and unprivileged by design, because the tool grants no
  // authority: the proposal sits on the public Agents board until the
  // operator approves or rejects it, and execution runs the desk's own
  // guarded paths. Identity is CLAIMED (a name), hashed into a stable id;
  // spoofing a name buys nothing a judged argument doesn't.
  server.registerTool(
    "meridian_propose_lp_action",
    {
      ...
      inputSchema: {
        ...
        maxUsd: z.number().min(25).max(500).optional(),
    ...
    async ({ kind, symbol, maxUsd, widthPct, rationale, agentName, dryRun }) => {
      ...
      return json({
        ...
        status: "pending",
        note: "The operator decides. Watch the Agents board on meridian402.xyz for the verdict.",
      });
    },
  );
